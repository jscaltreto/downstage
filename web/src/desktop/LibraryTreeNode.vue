...
<template>
  <div>
    <div
      ...
      :data-path="node.path"
      :data-kind="node.kind"
      @dragstart.stop="emit('dragStart', $event, node)"
      @dragend.stop="emit('dragEnd')"
      @dragover.stop="node.kind === 'folder' ? emit('dragOver', $event, node.path) : undefined"
      @drop.stop="node.kind === 'folder' ? emit('drop', $event, node.path) : undefined"
      @click="onRowClick"
