...
- Editor UI preferences (`theme`, `previewHidden`, `spellcheckDisabled`)
  round-trip through `env.getEditorPreferences` / `env.setEditorPreferences`.
  Both the browser env (`WebEnv` in `web-app.ts`) and the desktop env
  (`WailsBridge` in `desktop-app.ts`) implement this pair. The browser impl
  writes a single `downstage-editor-prefs` localStorage blob; the desktop
  impl round-trips the Go `Config.Preferences` struct in
  `internal/desktop/app.go` for the desktop-only settings. `Store` consumes
  the env methods and is the single owner of the browser-editor pref state.
- `Editor.vue` takes `previewHidden` and `spellcheckDisabled` as v-model
  ...
  browser-editor pref means: field on `EditorPreferences`, state on
  `Store`, v-model on `Editor.vue`. Desktop-only preferences live in the
  Wails bridge and should not be added to the browser editor store.
